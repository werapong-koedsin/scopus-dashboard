...
import { StatCard, QuartilePill, Spinner, MockBanner, SectionTitle } from '../components/ui'
...
export default function DashboardPage() {
  ...
  const byQuartile = useMemo(() => {
    ...
    return Object.entries(map)
      .map(([q, count]) => ({ q, count }))
      .sort((a, b) => ['Q1','Q2','Q3','Q4','Unknown'].indexOf(a.q) - ['Q1','Q2','Q3','Q4','Unknown'].indexOf(b.q))
  }, [publications])

  const recentPubs = useMemo(() =>
    [...publications].sort((a,b) => b.year - a.year || b.citations - a.citations).slice(0, 5)
  , [publications])
  ...
  return (
    <div className="space-y-7 fade-up">
      ...
      {/* Header */}
      <div>
        <h1 className="text-3xl font-extrabold text-slate-900 font-display leading-tight">
          Research Overview
        </h1>
        <p className="text-slate-400 mt-1 text-sm">ข้อมูลผลงานวิจัยจากฐานข้อมูล Scopus</p>
      </div>

      {/* KPI */}
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 fade-up fade-up-1">
        <StatCard value={stats?.total ?? 0} label="งานวิจัยทั้งหมด" color="#0EA5E9" />
        <StatCard value={stats?.q1 ?? 0} label="Q1 Papers"
          sub={`${stats?.total ? Math.round(stats.q1/stats.total*100) : 0}% ของทั้งหมด`} color="#059669" />
        <StatCard value={stats?.top10 ?? 0} label="Top 10% Journals" color="#7C3AED" />
        <StatCard value={(stats?.totalCitations ?? 0).toLocaleString()} label="Total Citations" color="#D97706" />
        <StatCard value={stats?.authors ?? 0} label="นักวิจัย" color="#0284C7" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 fade-up fade-up-2">
        ...
        <div className="card p-5 lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="by-year" className="text-xs text-sky-500 hover:text-sky-700 flex items-center gap-1 font-semibold">
        ...
        <div className="card p-5">
          <h3 className="font-bold text-slate-800 font-display mb-4">สัดส่วน Quartile</h3>
            ...
            {byQuartile.map(({ q, count }) => (
              <div key={q} className="flex items-center justify-between text-xs">
                <span className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-sm" style={{ background: Q_COLORS[q] }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 fade-up fade-up-3">
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="all" className="text-xs text-sky-500 hover:text-sky-700 flex items-center gap-1 font-semibold">
              ดูทั้งหมด <ArrowRight size={12} />
            </Link>
          </div>
          <div className="space-y-3">
            {recentPubs.map(pub => (
              <div key={pub.id} className="flex gap-3 items-start pb-3 border-b border-slate-50 last:border-0 last:pb-0">
                <span className="text-xl font-extrabold text-slate-100 font-display leading-none mt-0.5 w-10 shrink-0 text-right">
                  {pub.year}
                </span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-800 leading-snug line-clamp-2">{pub.title}</p>
                  <p className="text-xs text-slate-400 mt-0.5 truncate">{pub.authors}</p>
                  <div className="mt-1.5">
                    <QuartilePill q={pub.quartile} isTop10={pub.isTop10} />
                  </div>
                </div>
                <span className="shrink-0 text-xs font-bold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded-lg">
                  ⌟{pub.citations}
                </span>
              </div>
            ))}
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="by-author" className="text-xs text-sky-500 hover:text-sky-700 flex items-center gap-1 font-semibold">
              ดูทั้งหมด <ArrowRight size={12} />
            </Link>
          </div>
          <div className="space-y-3">
            {topAuthors.map((a, i) => {
              ...
              return (
                <div key={a.id} className="flex items-center gap-3">
                  <span className="w-7 h-7 rounded-full bg-sky-100 text-sky-700 text-xs font-extrabold
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-[10px] text-slate-400">{pubs.length} papers · {q1} Q1</p>
                  </div>
                </div>
              )
            })}
