...
const SORT_OPTIONS = [
  { value: 'name',      label: '🔤 รายชื่อ',   desc: 'A → Z' },
...
export default function ByAuthorPage() {
  ...
  const [sortBy, setSortBy] = useState('name')  // ← default: เรียงตามรายชื่อ
  ...
  const authorStats = useMemo(() => {
    return authors
      ...
      .sort((x, y) => {
        switch (sortBy) {
          case 'name':
            // เรียง A → Z ตามชื่ออังกฤษ (nameEn) หรือชื่อไทย
            return (x.author.nameEn || x.author.name)
              .localeCompare(y.author.nameEn || y.author.name, 'en', { sensitivity: 'base' })

          case 'top10': {
            // เรียงตามจำนวน Top 10% papers มากก่อน
            const xTop = x.pubs.filter(p => p.isTop10).length
            const yTop = y.pubs.filter(p => p.isTop10).length
            if (yTop !== xTop) return yTop - xTop
            // ถ้าเท่ากัน → citations รองลงมา
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1
            ...
            {SORT_OPTIONS.map(opt => (
              <button
                ...
                className={`shrink-0 snap-start px-3 py-1.5 rounded-lg text-xs
